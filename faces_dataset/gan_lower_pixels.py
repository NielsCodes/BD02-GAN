...
images = [np.array((Image.open(path).crop(crop)).resize((32,32))) for path in select_path]
...
generator.add(Dense(2*2*256,input_shape=[noise_shape]))
generator.add(Reshape([2,2,256]))
generator.add(Conv2DTranspose(128, kernel_size=4, strides=2, padding="same"))
generator.add(LeakyReLU(alpha=0.8))
...
discriminator.add(Conv2D(32, kernel_size=4, strides=2, padding="same", input_shape=[32, 32, 3]))
discriminator.add(Conv2D(64, kernel_size=4, strides=2, padding="same"))
discriminator.add(LeakyReLU(0.8)) 
...
for i in range(5):
    ...
    for k in range(20):
            ...
            plt.subplot(5, 4, k+1)
            plt.imshow(new_img[k].reshape(32,32,3))  
            plt.xticks([])
            plt.yticks([])
